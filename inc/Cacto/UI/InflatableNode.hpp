...
namespace cacto
{
    ...
    public:
        ...
        static sf::Vector2f compact(const Shared<Node> &node);
        static sf::Vector2f compactChildren(const Shared<const Node> &node);

        static sf::Vector2f inflate(const Shared<Node> &node, const sf::Vector2f &containerSize = {0, 0});
        static sf::Vector2f inflateChildren(const Shared<const Node> &node, const sf::Vector2f &containerSize = {0, 0});

        static void place(const Shared<Node> &node, const sf::Vector2f &position = {0, 0});
        static void placeChildren(const Shared<const Node> &node, const sf::Vector2f &position = {0, 0});
